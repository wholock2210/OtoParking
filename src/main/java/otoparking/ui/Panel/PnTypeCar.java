...
public class PnTypeCar extends JPanel {
	...
	private void DrawDetails(JPanel pnDetailsInfo){
		JLabel title = new JLabel("Loại xe");
	...
	private void onTableSelected(ListSelectionEvent e){
		if (!e.getValueIsAdjusting()) {
			...
			if (selectedRow != -1) {
				...
				currentTypeCar = tcDAO.FirstOfDefault(id);

				if(currentTypeCar != null){
					tfId.setText(String.valueOf(currentTypeCar.getId()));
					tfName.setText(currentTypeCar.getName());
				}
			}
		}
	}
	...
	private void onJBCreate(){
		...
		if(
			...
			boolean result = tcDAO.Insert(newTypeCar);
			if(result)
				JOptionPane.showMessageDialog(
					this,
					"Thêm loại " + name + " thành công",
					"Thông Báo",
	...
	private void onJBDelete(){
		...
		int result = JOptionPane.showConfirmDialog(this, 
			"bạn có chắc muốn xóa loại : " + currentTypeCar.getName() + " không?",
			"xác nhận xóa",
			JOptionPane.YES_NO_OPTION,
			JOptionPane.WARNING_MESSAGE
		);
		if(result == JOptionPane.YES_OPTION){
			boolean isDel = tcDAO.Delete(currentTypeCar);
			if(isDel)
				JOptionPane.showMessageDialog(
					this,
					"xóa loại " + currentTypeCar.getName() + " thành công",
					"Thông Báo",
					JOptionPane.INFORMATION_MESSAGE
				);
			else
				JOptionPane.showMessageDialog(
					this,
					"xóa loại " + currentTypeCar.getName() + " thất bại",
					"Thông Báo",
					JOptionPane.ERROR_MESSAGE
				);
			currentTypeCar = null;
	...
	private void onJBEdit(){
		if(currentTypeCar == null){
			JOptionPane.showMessageDialog(this, "vui lòng chọn loại cần sửa!", "thông báo", JOptionPane.WARNING_MESSAGE);
			return;
		}
		String name;
		name = tfName.getText();
		if(
			...
			TypeCar newTypeCar = new TypeCar(
				currentTypeCar.getId(),
				name
			);

			boolean result = tcDAO.Update(newTypeCar);
			if(result)
				JOptionPane.showMessageDialog(
					this,
					"Sửa loại " + name + " thành công",
					"Thông Báo",
					JOptionPane.INFORMATION_MESSAGE
				);
			else
	...
	public PnTypeCar(JPanel pnContent, CardLayout cl) {
		...
		JButton btnBack = new JButton("Trở về trang quản lý xe", backIcon);
